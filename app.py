...
#from Picture_subject import picture_subject モジュール化したものから取り出す方法
...
class Pen:
    def __init__(self):
        self.color = (127, 0, 255)  # ペンの色(BGR)
        self.erase_color = (0, 0, 0)  # ペンの消しゴムの色(paint_canvasの背景と同じ色)
        self.thickness = 15  # ペンの太さ
...
heart_stamp_image = cv.imread("Heart.png", cv.IMREAD_UNCHANGED)  # 背景が透明な画像（.png）
good_stamp_image = cv.imread("Good.png", cv.IMREAD_UNCHANGED)  # 背景が透明な画像（.png）
background_image = cv.imread('backgroundsample.png')
...
def main():
    ...
    old_right_hand_sign_id = None
    left_hand_sign_id = None
    left_point_landmark = [0, 0]
    right_point_landmark = [0, 0]
    stamp_positions_heart = []
    stamp_positions_good = []
    stamp_angle_heart = []
    stamp_angle_good = []
    calc_angle_heart = None
    calc_angle_good = None
    tip_left_thumb = None
    base_left_thumb = None
    ...
    while True:
        ...
        # 背景画像を読み込み、カメラ映像と同じサイズにリサイズ
        #カメラじゃなく背景画像に合わせるように後で変更する
        background_image_resized = cv.resize(background_image, (image.shape[1], image.shape[0]))

        # 背景画像とペイント部分を合成
        paint_canvas2gray = cv.cvtColor(paint_canvas, cv.COLOR_BGR2GRAY)  # グレースケール変換
        ret, mask = cv.threshold(paint_canvas2gray, 1, 255, cv.THRESH_BINARY)
        mask_inv = cv.bitwise_not(mask)  # 白背景にする

        # 背景画像を使って合成
        image_src = cv.bitwise_and(background_image_resized, background_image_resized, mask=mask_inv)  # 背景画像にペイントを反映
        paint_canvas = cv.bitwise_and(paint_canvas, paint_canvas, mask=mask)  # ペイント部分を抽出
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if info_text == "Left":
                    ...
                    if left_hand_sign_id == 5:  # OKサイン
                        left_point_landmark = landmark_list[8]  # 人差指座標
                    elif left_hand_sign_id == 4:  # グッドサイン
                        left_point_landmark = landmark_list[8]  # 人差指座標
                        tip_left_thumb = landmark_list[4]
                        base_left_thumb = landmark_list[1]
                    elif left_hand_sign_id == 3:  # ハートサイン
                        left_point_landmark = landmark_list[8]  # 人差指座標
                        tip_left_thumb = landmark_list[4]
                        base_left_thumb = landmark_list[1]
                    elif left_hand_sign_id == 2:  # 指差しサイン
                        left_point_landmark = landmark_list[8]  # 人差指座標
                    elif left_hand_sign_id == 1:  # グーの形のサイン
                        left_point_landmark = landmark_list[8]  # 人差指座標
                    elif left_hand_sign_id == 0:  # パーの形のサイン
                        left_point_landmark = landmark_list[8]  # 人差指座標
                elif info_text == "Right":
                    right_hand_sign_id = hand_sign_id
                    if right_hand_sign_id == 5:  # OKサイン
                        right_point_landmark = landmark_list[8]  # 人差指座標
                        if old_right_hand_sign_id != 5:
                            right_hand_position_saved = True
                        else:
                            right_hand_position_saved = None
                    elif right_hand_sign_id == 4:  # グッドサイン
                        right_point_landmark = landmark_list[8]  # 人差指座標
                    elif right_hand_sign_id == 3:  # ハートサイン
                        right_point_landmark = landmark_list[8]  # 人差指座標
                    elif right_hand_sign_id == 2:  # 指差しサイン
                        right_point_landmark = landmark_list[8]  # 人差指座標
                    elif right_hand_sign_id == 1:  # グーの形のサイン
                        right_point_landmark = landmark_list[8]  # 人差指座標
                    elif right_hand_sign_id == 0:  # パーの形のサイン
                        right_point_landmark = landmark_list[8]  # 人差指座標
                else:
                    left_hand_sign_id = None
                    right_hand_sign_id = None
                ...
                print(tip_left_thumb)
                print(base_left_thumb)

                #point_history.append(landmark_list)
                point_history.append(right_point_landmark)
                hand_gesture_history.append(hand_sign_id)

                print(old_right_hand_sign_id)
                print(right_hand_sign_id)
                ...
                # 描画
                if (info_text == "Left"):
                    if (left_hand_sign_id == 0):
                        pass  # パーだったら何もしない（ポインター的なものを表示する必要はあり）
                    elif (left_hand_sign_id == 1):
                        pass
                    elif (left_hand_sign_id == 2):
                        pass
                    elif (left_hand_sign_id == 3):
                        #calc_angle_heart = calculation_angle(tip_left_thumb, base_left_thumb)
                        dst = overlay_stamp(dst, "heart", left_point_landmark, 120)
                    elif (left_hand_sign_id == 4):
                        #calc_angle_good = calculation_angle(tip_left_thumb, base_left_thumb)
                        dst = overlay_stamp(dst, "good", left_point_landmark, 150)
                    elif (left_hand_sign_id == 5):
                            pass
                        
                if (info_text == "Right"):
                    if len(hand_gesture_history) > 1:
                        old_right_hand_sign_id = hand_gesture_history[1]
                    if (right_hand_sign_id == 0):
                        pass
                    elif (right_hand_sign_id == 1):
                        pass
                    elif (right_hand_sign_id == 2):
                            if len(hand_gesture_history) > 1:
                                if (hand_gesture_history[1] == 2):
                                    paint_canvas = draw_latest_point_line(
                                        paint_canvas, point_history, pen.thickness, pen.color)
                    elif (right_hand_sign_id == 3):
                            pass
                    elif (right_hand_sign_id == 4):
                        pass
                    elif (right_hand_sign_id == 5):
                        # 右手がOKの時、左手の位置を固定
                        if right_hand_position_saved:
                            if left_hand_sign_id == 3:
                                stamp_positions_heart.append(left_point_landmark)
                                stamp_angle_heart.append(calc_angle_heart)
                            elif left_hand_sign_id == 4:
                                stamp_positions_good.append(left_point_landmark)
                                stamp_angle_good.append(calc_angle_good)
        ...
        for pos in stamp_positions_heart:
            dst = overlay_stamp(dst, "heart", pos, 120)
            #dst = overlay_stamp(dst, "heart", stamp_positions_heart[pos], 120, stamp_angle_heart[pos])
        for pos in stamp_positions_good:
            dst = overlay_stamp(dst, "good", pos, 150)

        dst = draw_info(dst, fps, mode, number)

        #game_image = draw_UI_in_game(game_image)
        #game_image = scene_transition(game_image)

        dst = draw_cursor(dst, point_history, history_length)
        # 画面反映 #############################################################
        # rキーで切り替えできる
        #print(game_mode)
        if (debugmode):
            ...
            cv.imshow('Hand Gesture Recognition', dst)
...
# 日本語を描画する関数
def putText_japanese(image, text, position, size, color):
    # Notoフォントとする
...
def overlay_stamp(image, stamp_type, position, size):
    # スタンプタイプに応じて表示する画像を設定
    if stamp_type == "heart":
        stamp = heart_stamp_image  # Heart画像を使用
    elif stamp_type == "good":
        stamp = good_stamp_image  # Good画像を使用
    else:
        return image  # 不正なスタンプタイプの場合、変更なしで画像を返す
    
    # スタンプの元のサイズを取得
    stamp_height, stamp_width = stamp.shape[:2]
    h, w = image.shape[:2]  # 背景画像の高さと幅
    ...
    # リサイズされたスタンプ画像を生成
    resized_stamp = cv.resize(stamp, (new_width, new_height))
    resized_stamp_height, resized_stamp_width = resized_stamp.shape[:2]  # リサイズ後の高さと幅

    # もしスタンプがRGBA形式なら、アルファチャネルを使って合成
    if resized_stamp.shape[2] == 4:
        # アルファチャネル（透過部分）を分離
        alpha_channel = resized_stamp[:, :, 3] / 255.0  # [0, 1]に正規化
        stamp_rgb = resized_stamp[:, :, :3]  # RGB部分のみを取り出す
    else:
        alpha_channel = np.ones((resized_stamp_height, resized_stamp_width), dtype=np.uint8) * 255
        stamp_rgb = resized_stamp  # アルファチャネルがない場合、そのままRGB部分を使用

    # スタンプの領域が背景画像内に収まるように調整
    if x + resized_stamp_width > w or y + resized_stamp_height > h:
        return image  # スタンプが範囲外なら何も変更しない

    # 背景画像をコピーしてスタンプを重ねる
    image = image.copy()

    # スタンプを背景に重ねる
    for c in range(0, 3):  # RGBの各チャネルに対して
        # 背景画像の該当領域
        background_region = image[y:y+resized_stamp_height, x:x+resized_stamp_width, c]
        
        # 合成：アルファチャネルを使って重ねる
        image[y:y+resized_stamp_height, x:x+resized_stamp_width, c] = (
            alpha_channel * stamp_rgb[:, :, c] + (1 - alpha_channel) * background_region
        ).astype(np.uint8)  # 合成結果を整数値にキャスト
